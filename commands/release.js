...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (releaseChannel && releaseChannel.isTextBased()) {
      await releaseChannel.send({ embeds: [embed], allowedMentions: { parse: [], roles: [] } }).catch(() => null);
    }

    await interaction.reply({ content: `✅ ${targetMember.user.tag} has been released from ${updatedTeam.teamName}.`, embeds: [embed], flags: 64, allowedMentions: { parse: ['users'], roles: [] } });
  },
};
